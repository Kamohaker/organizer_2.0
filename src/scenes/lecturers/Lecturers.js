...
import { Pressable, Input, Box,Divider, FlatList, Avatar, HStack,Fab,Text, Icon, NativeBaseProvider } from "native-base";
    ...
    <NativeBaseProvider>
      <LinearGradient
        ...
        <FlatList data={data.filter(checkTitles)}  renderItem={({item}) => 
          <Pressable onPress={()=>{navigation.navigate('Edytuj Prowadzących',{nazwa:item.nazwa,stopien:item.stopien,num_pok:item.num_pokoju})}}>   
        ...
        <Fab  shadow={4} bgColor={colors.darkGreyBlue} 
          onPress={() => navigation.navigate('Dodaj Prowadzących')}
